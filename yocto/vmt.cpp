...
namespace toolkit
{
	...
	unsigned int count_funcs(void** vmt)
	{
		unsigned int i = -1;
		do ++i; while (vmt[i]);
		return i;
	}
	int find_function(void** vmt, void* func, unsigned int vfuncs)
	{
		if (!vfuncs) vfuncs = count_funcs(vmt);
	...
	// vmt_base_manager
	vmt_base_manager& vmt_base_manager::init(void* inst, unsigned int offset, unsigned int vfuncs)
	{
	...
	void vmt_base_manager::EraseHooks()
	{
		unsigned int i = 0;
		void** vmt = _array + 3;
		do vmt[i] = _oldvmt[i];
	...
	// vmt_base_pointer
	vmt_base_pointer& vmt_base_pointer::init(void* inst, CallGateFn gate, unsigned int vfuncs)
	{
		_inst = inst;
		_gate = gate;
		void** vmt = *(void***)inst;
	...
	void* __fastcall vmt_base_pointer::FindCallOffset(vmt_base_pointer* _this, unsigned char* addr)
	{
		unsigned int vfn;
...
#if defined(_M_IX86)
	__declspec(naked) void __fastcall vmt_base_pointer::CallGate_thiscall()
	{
		__asm
		{
	...
	__declspec(naked) void __fastcall vmt_base_pointer::CallGate_stdcall()
	{
		__asm
		{
